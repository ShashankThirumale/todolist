...
app.post("/render", (req, res) => {
    pool.getConnection(function (err, db) {
        if (err) {
            ...
            const email = req.body.email;
            db.query(
                "SELECT TaskId AS id, Task AS task, CASE Complete WHEN 1 THEN true WHEN 0 THEN false END AS complete, false AS isEditing FROM TodoList.Tasks WHERE UserEmail = ?;",
                email,
                (err2, result) => {
                    if (err2) {
                        res.send({ err: err2 });
